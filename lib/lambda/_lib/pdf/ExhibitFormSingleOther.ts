 ...
 */
export class ExhibitFormSingleOther extends PdfForm implements IPdfForm {
  private baseForm:ExhibitForm
  private font:PDFFont;
  private boldfont:PDFFont;

  public static getBlankForm = (): IPdfForm => {
    return new ExhibitFormSingleOther(ExhibitForm.getBlankForm(
  ...
  public static getInstance = (parms:ExhibitFormParms): IPdfForm => {
    ...
    return new ExhibitFormSingleOther(new ExhibitForm(parms));
  }
  ...
  private drawIntro = async () => {
    ...
    await drawWrappedText({ 
      text: isBlankForm ? 
        `<b>This “<u>Single-Entity Exhibit Form</u>” is incorporated into ${fullname} Consent Form, at:</b>` : 
        `<b>This “<u>Single-Entity Exhibit Form</u>” is incorporated into ${fullname} Consent Form, attached to this Exhibit Form.</b>`, 
...
if(args.length > 2 && args[2].replace(/\\/g, '/').endsWith('lib/lambda/_lib/pdf/ExhibitFormSingleOther.ts')) {
  ...
  (async () => {
    ...
    const form = testBlankForm ?
      ExhibitFormSingleOther.getBlankForm() :
      ExhibitFormSingleOther.getInstance(SampleExhibitFormParms([ getSampleAffiliates().academic1 ]));

    await form.writeToDisk('./lib/lambda/_lib/pdf/ExhibitFormSingleOther.pdf');
    console.log(`done`);
