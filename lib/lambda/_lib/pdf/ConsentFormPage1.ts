  ...
  private drawBody = async () => {
    ...
    await page.drawRectangle({
      text: [
        ...
        'Form<sup>1</sup> to request a disclosure);', '',
    ...
    await page.drawWrappedText({
      ...
      'contact for each of my Consent Recipients <sup>5</sup> each time any ETT-Registered Entity<sup>7</sup></b>makes a request for disclosures.' + 
...
if(args.length > 2 && args[2].replace(/\\/g, '/').endsWith('lib/lambda/_lib/pdf/ConsentFormPage1.ts')) {
