...
export const handler = async(event) => {
  ...
  try {
    ...
    if(hasAtLeaseOnePlaceholder(content)) {
      /**
       * Take great care here: If you don't replace ALL placeholders checked for by hasAtLeastOnePlaceholder, 
...
const hasAtLeaseOnePlaceholder = content => {
  return content.includes('CLIENT_ID_PLACEHOLDER') || 
       content.includes('COGNITO_DOMAIN_PLACEHOLDER') || 
       content.includes('USER_POOL_REGION_PLACEHOLDER') || 
       content.includes('REDIRECT_URI_PLACEHOLDER') || 
       content.includes('HELLO_WORLD_API_URI_PLACEHOLDER');
}
